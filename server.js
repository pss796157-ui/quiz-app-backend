...
app.post('/auth/login', async (req, res) => {
    ...
    try {
        ...
        if (user) {
            // Security Check: Verify deviceId
            if (user.deviceId && user.deviceId !== deviceId) {
                console.log('Login Failed: Unauthorized device for', email);
                return res.status(403).json({ message: "Login blocked: This account is linked to another device." });
            }
